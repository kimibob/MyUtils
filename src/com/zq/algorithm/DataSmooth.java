package com.zq.algorithm;

import java.util.Arrays;

/**
 *
 * 移动窗口拟和多项式平滑(最小二乘多项式平滑)(Savitzky-Golay Smoothing) 
 * 思路:使用最小二乘法将数据的一个小窗口回归到多项式上，然后使用多项式来估计窗口中心的点。最后窗口向前移动一个数据点，重复这个过程。
 * 	        这样继续下去，直到每个点相对于邻居都进行了最佳调整。
 * 假设数据为x，选定的平滑窗口大小为m (其必须为奇数，这里以7为例)
 * 多项式次数为n，这里以3为例，当前平滑的点为x0，前3个点分别记为：x-3，x-2，x-1，以及后三个点记为：x1，x2，x3。
 * 平滑后的数据量与原数据量相同，不会掐头去尾
 * Modified log:<br>
 * ------------------------------------------------------<br>
 * Ver.		Date		Author			Description<br>
 * ------------------------------------------------------<br>
 * 1.0		2018年7月26日 下午5:06:31		zhouqiang		created.<br>
 */

public class DataSmooth {
	public static void main(String[] args) {

		double[] input = { 1, 1, 1, 1, 1, 1, 2, 1, 1, 2, 3, 11, 6, 13, 22, 26,
				32, 44, 37, 44, 64, 71, 100, 101, 152, 159, 147, 215, 168, 152,
				213, 227, 203, 184, 151, 127, 127, 105, 125, 113, 111, 79, 74,
				67, 38, 33, 29, 24, 29, 41, 21, 9, 10, 11, 10, 8, 4, 1, 1, 2,
				2, 8, 1, 4, 1 };
		
		System.out.println(Arrays.toString(linearSmooth7(input, input.length)));
	}

	/*
	 * 直线滑动平均法,窗口大小3(三点线性平滑)
	 */
	private static double[] linearSmooth3(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 3) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (5.0 * in[0] + 2.0 * in[1] - in[2]) / 6.0;

			for (i = 1; i <= N - 2; i++) {
				out[i] = (in[i - 1] + in[i] + in[i + 1]) / 3.0;
			}

			out[N - 1] = (5.0 * in[N - 1] + 2.0 * in[N - 2] - in[N - 3]) / 6.0;
		}
		return out;
	}

	/*
	 * 直线滑动平均法,窗口大小5
	 */
	private static double[] linearSmooth5(double[] in, int N) {
		int i;
		double[] out = new double[N];

		if (N < 5) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (3.0 * in[0] + 2.0 * in[1] + in[2] - in[4]) / 5.0;
			out[1] = (4.0 * in[0] + 3.0 * in[1] + 2 * in[2] + in[3]) / 10.0;
			for (i = 2; i <= N - 3; i++) {
				out[i] = (in[i - 2] + in[i - 1] + in[i] + in[i + 1] + in[i + 2]) / 5.0;
			}
			out[N - 2] = (4.0 * in[N - 1] + 3.0 * in[N - 2] + 2 * in[N - 3] + in[N - 4]) / 10.0;
			out[N - 1] = (3.0 * in[N - 1] + 2.0 * in[N - 2] + in[N - 3] - in[N - 5]) / 5.0;
		}

		return out;
	}

	/*
	 * 直线滑动平均法,窗口大小7
	 */
	private static double[] linearSmooth7(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 7) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (13.0 * in[0] + 10.0 * in[1] + 7.0 * in[2] + 4.0 * in[3]
					+ in[4] - 2.0 * in[5] - 5.0 * in[6]) / 28.0;

			out[1] = (5.0 * in[0] + 4.0 * in[1] + 3 * in[2] + 2 * in[3] + in[4] - in[6]) / 14.0;

			out[2] = (7.0 * in[0] + 6.0 * in[1] + 5.0 * in[2] + 4.0 * in[3]
					+ 3.0 * in[4] + 2.0 * in[5] + in[6]) / 28.0;

			for (i = 3; i <= N - 4; i++) {
				out[i] = (in[i - 3] + in[i - 2] + in[i - 1] + in[i] + in[i + 1]
						+ in[i + 2] + in[i + 3]) / 7.0;
			}

			out[N - 3] = (7.0 * in[N - 1] + 6.0 * in[N - 2] + 5.0 * in[N - 3]
					+ 4.0 * in[N - 4] + 3.0 * in[N - 5] + 2.0 * in[N - 6] + in[N - 7]) / 28.0;

			out[N - 2] = (5.0 * in[N - 1] + 4.0 * in[N - 2] + 3.0 * in[N - 3]
					+ 2.0 * in[N - 4] + in[N - 5] - in[N - 7]) / 14.0;

			out[N - 1] = (13.0 * in[N - 1] + 10.0 * in[N - 2] + 7.0 * in[N - 3]
					+ 4 * in[N - 4] + in[N - 5] - 2 * in[N - 6] - 5 * in[N - 7]) / 28.0;
		}
		return out;
	}

	/*
	 * 二次函数拟合平滑(多项式次数为n=2,平滑窗口大小为m=5)
	 */
	private static double[] quadraticSmooth5(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 5) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (31.0 * in[0] + 9.0 * in[1] - 3.0 * in[2] - 5.0 * in[3] + 3.0 * in[4]) / 35.0;
			out[1] = (9.0 * in[0] + 13.0 * in[1] + 12 * in[2] + 6.0 * in[3] - 5.0 * in[4]) / 35.0;
			for (i = 2; i <= N - 3; i++) {
				out[i] = (-3.0 * (in[i - 2] + in[i + 2]) + 12.0
						* (in[i - 1] + in[i + 1]) + 17 * in[i]) / 35.0;
			}
			out[N - 2] = (9.0 * in[N - 1] + 13.0 * in[N - 2] + 12.0 * in[N - 3]
					+ 6.0 * in[N - 4] - 5.0 * in[N - 5]) / 35.0;
			out[N - 1] = (31.0 * in[N - 1] + 9.0 * in[N - 2] - 3.0 * in[N - 3]
					- 5.0 * in[N - 4] + 3.0 * in[N - 5]) / 35.0;
		}
		return out;
	}

	/*
	 * 二次函数拟合平滑(多项式次数为n=2,平滑窗口大小为m=7)
	 */
	private static double[] quadraticSmooth7(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 7) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (32.0 * in[0] + 15.0 * in[1] + 3.0 * in[2] - 4.0 * in[3]
					- 6.0 * in[4] - 3.0 * in[5] + 5.0 * in[6]) / 42.0;

			out[1] = (5.0 * in[0] + 4.0 * in[1] + 3.0 * in[2] + 2.0 * in[3]
					+ in[4] - in[6]) / 14.0;

			out[2] = (1.0 * in[0] + 3.0 * in[1] + 4.0 * in[2] + 4.0 * in[3]
					+ 3.0 * in[4] + 1.0 * in[5] - 2.0 * in[6]) / 14.0;
			for (i = 3; i <= N - 4; i++) {
				out[i] = (-2.0 * (in[i - 3] + in[i + 3]) + 3.0
						* (in[i - 2] + in[i + 2]) + 6.0
						* (in[i - 1] + in[i + 1]) + 7.0 * in[i]) / 21.0;
			}
			out[N - 3] = (1.0 * in[N - 1] + 3.0 * in[N - 2] + 4.0 * in[N - 3]
					+ 4.0 * in[N - 4] + 3.0 * in[N - 5] + 1.0 * in[N - 6] - 2.0 * in[N - 7]) / 14.0;

			out[N - 2] = (5.0 * in[N - 1] + 4.0 * in[N - 2] + 3.0 * in[N - 3]
					+ 2.0 * in[N - 4] + in[N - 5] - in[N - 7]) / 14.0;

			out[N - 1] = (32.0 * in[N - 1] + 15.0 * in[N - 2] + 3.0 * in[N - 3]
					- 4.0 * in[N - 4] - 6.0 * in[N - 5] - 3.0 * in[N - 6] + 5.0 * in[N - 7]) / 42.0;
		}
		return out;
	}

	/*
	 * 三次函数拟合平滑(多项式次数为n=3,平滑窗口大小为m=5)
	 */
	private static double[] cubicSmooth5(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 5) {
			for (i = 0; i <= N - 1; i++)
				out[i] = in[i];
		}

		else {
			out[0] = (69.0 * in[0] + 4.0 * in[1] - 6.0 * in[2] + 4.0 * in[3] - in[4]) / 70.0;
			out[1] = (2.0 * in[0] + 27.0 * in[1] + 12.0 * in[2] - 8.0 * in[3] + 2.0 * in[4]) / 35.0;
			for (i = 2; i <= N - 3; i++) {
				out[i] = (-3.0 * (in[i - 2] + in[i + 2]) + 12.0
						* (in[i - 1] + in[i + 1]) + 17.0 * in[i]) / 35.0;
			}
			out[N - 2] = (2.0 * in[N - 5] - 8.0 * in[N - 4] + 12.0 * in[N - 3]
					+ 27.0 * in[N - 2] + 2.0 * in[N - 1]) / 35.0;
			out[N - 1] = (-in[N - 5] + 4.0 * in[N - 4] - 6.0 * in[N - 3] + 4.0
					* in[N - 2] + 69.0 * in[N - 1]) / 70.0;
		}
		return out;
	}

	/*
	 * 三次函数拟合平滑(多项式次数为n=3,平滑窗口大小为m=7)
	 */
	private static double[] cubicSmooth7(double[] in, int N) {
		int i;
		double[] out = new double[N];
		if (N < 7) {
			for (i = 0; i <= N - 1; i++) {
				out[i] = in[i];
			}
		} else {
			out[0] = (39.0 * in[0] + 8.0 * in[1] - 4.0 * in[2] - 4.0 * in[3]
					+ 1.0 * in[4] + 4.0 * in[5] - 2.0 * in[6]) / 42.0;
			out[1] = (8.0 * in[0] + 19.0 * in[1] + 16.0 * in[2] + 6.0 * in[3]
					- 4.0 * in[4] - 7.0 * in[5] + 4.0 * in[6]) / 42.0;
			out[2] = (-4.0 * in[0] + 16.0 * in[1] + 19.0 * in[2] + 12.0 * in[3]
					+ 2.0 * in[4] - 4.0 * in[5] + 1.0 * in[6]) / 42.0;
			for (i = 3; i <= N - 4; i++) {
				out[i] = (-2.0 * (in[i - 3] + in[i + 3]) + 3.0
						* (in[i - 2] + in[i + 2]) + 6.0
						* (in[i - 1] + in[i + 1]) + 7.0 * in[i]) / 21.0;
			}
			out[N - 3] = (-4.0 * in[N - 1] + 16.0 * in[N - 2] + 19.0
					* in[N - 3] + 12.0 * in[N - 4] + 2.0 * in[N - 5] - 4.0
					* in[N - 6] + 1.0 * in[N - 7]) / 42.0;
			out[N - 2] = (8.0 * in[N - 1] + 19.0 * in[N - 2] + 16.0 * in[N - 3]
					+ 6.0 * in[N - 4] - 4.0 * in[N - 5] - 7.0 * in[N - 6] + 4.0 * in[N - 7]) / 42.0;
			out[N - 1] = (39.0 * in[N - 1] + 8.0 * in[N - 2] - 4.0 * in[N - 3]
					- 4.0 * in[N - 4] + 1.0 * in[N - 5] + 4.0 * in[N - 6] - 2.0 * in[N - 7]) / 42.0;
		}
		return out;
	}
}
